...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        curr_place, counter = self.place, 0
        while not curr_place.is_hive:
            if curr_place.bees and self.min_range <= counter <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            [b.reduce_health(amount + self.damage) for b in bees]
        else:
            [b.reduce_health(amount) for b in bees]
        super().reduce_health(amount)


class WallAnt(Ant):
    """WallAnt sits still and does nothing besides soak damage."""
...
class HungryAnt(Ant):
    """HungryAnt eats a random bee in its place whole with a 3 turn cooldown."""
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and does 1 damage to bees."""
    ...
    def action(self, gamestate):
        if self.ant_contained:
            self.ant_contained.action(gamestate)
        bees = self.place.bees.copy()
        [b.reduce_health(self.damage) for b in bees]
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a ThrowerAnt that can survive in water."""
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, gamestate):
        ...
        def buff_ants(place):
            ...
            if place.ant:
                place.ant.buff()
            buff_ants(place.exit)

        buff_ants(self.place.exit)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        backward = self.place.entrance

        if self.blocked():
            self.sting(self.place.ant)
        elif self.health > 0 and destination is not None:
            if self.slow_length and self.scare_length:
                if gamestate.time % 2 == 0:
                    self.move_to(backward)
                    self.scare_length -= 1
                self.slow_length -= 1
            elif self.slow_length:
                if gamestate.time % 2 == 0:
                    self.move_to(destination)
                self.slow_length -= 1
            elif self.scare_length:
                if not backward.is_hive:
                    self.move_to(backward)
                self.scare_length -= 1
            else:
                self.move_to(destination)
